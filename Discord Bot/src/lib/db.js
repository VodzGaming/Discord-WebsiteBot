...
// ---- Levels helpers ----
export function getLevelRow(guild_id, user_id){
  const row = db.prepare('SELECT * FROM levels WHERE guild_id=? AND user_id=?').get(guild_id, user_id);
  return row || { guild_id, user_id, xp: 0, level: 0, last_award_at: 0 };
}

export function setLevelRow({ guild_id, user_id, xp=0, level=0, last_award_at=null }){
  db.prepare(`INSERT INTO levels (guild_id, user_id, xp, level, last_award_at)
              VALUES (@guild_id,@user_id,@xp,@level,@last_award_at)
              ON CONFLICT(guild_id,user_id) DO UPDATE SET xp=@xp, level=@level, last_award_at=@last_award_at`).run({ guild_id, user_id, xp, level, last_award_at });
}

function levelForXp(xp){
  // Simple quadratic curve: next level cost grows roughly as 50 * level
  // Total XP to reach level L ~ 25 * L * (L+1)
  let l = 0; let need = 25;
  let remaining = Math.max(0, xp|0);
  while(remaining >= need && l < 1000){ remaining -= need; l++; need = 25 * (l + 1); }
  return l;
}

export function addXp(guild_id, user_id, delta){
  const now = Date.now();
  const cur = getLevelRow(guild_id, user_id);
  const xp = Math.max(0, (cur.xp||0) + (delta||0));
  const level = levelForXp(xp);
  const leveledUp = level > (cur.level||0);
  setLevelRow({ guild_id, user_id, xp, level, last_award_at: now });
  return { xp, level, leveledUp };
}

// ---- Tracked streams helpers ----
export function getTrackedStreams(platform=null){
  if(platform){ return db.prepare('SELECT * FROM tracked_streams WHERE platform=?').all(platform); }
  return db.prepare('SELECT * FROM tracked_streams').all();
}

export function updateTrackedStreamStatus(id, { live=null, announceAt=null }){
  const fields = [];
  const params = { id };
  if(live !== null && live !== undefined){ fields.push('last_live_status=@live'); params.live = live ? 1 : 0; }
  if(announceAt){ fields.push('last_announcement_at=@announceAt'); params.announceAt = announceAt; }
  if(!fields.length) return;
  const sql = `UPDATE tracked_streams SET ${fields.join(', ')} WHERE id=@id`;
